...
export function createRecoveryService({ automationLedger, automationRunner, domainReconcilers = {}, clock } = {}) {
  ...
  function toCase(run) {
    ...
    const closedManually = closures.get(run.runId) ?? null;
  ...
  return Object.freeze({
    async listCases(filters = {}) {
      ...
      const cases = runs.filter((run) => run.status !== 'running').map(toCase);
    ...
    async proposeNextAction(caseId) {
      ...
      return toCase(run).allowedActions[0] ?? null;
    },
