...
1. Aller sur [console.cloud.google.com](https://console.cloud.google.com), se connecter avec `sourireconcept@gmail.com`.
...
4. **APIs et services → Écran de consentement OAuth** :
   ...
   - Ajouter `sourireconcept@gmail.com` comme **utilisateur de test** (évite la revue de vérification Google pour un usage personnel).
...
- Le navigateur par défaut s'ouvre sur l'écran de consentement Google — se connecter avec `sourireconcept@gmail.com`, accepter les permissions demandées (Gmail, Calendrier, Contacts, Tâches).
...
1. Se connecter sur la page officielle Google Home Developer avec `sourireconcept@gmail.com`.
