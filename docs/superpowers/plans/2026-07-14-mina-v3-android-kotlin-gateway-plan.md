...
- [x] Write failing tests where ADB reports `L2N4C19515006599` and `192.168.1.16:5555` for the same signed `deviceId`; assert one physical device with ordered transports USB then LAN.
...
Réel (2026-07-16, matériel physique désormais disponible) : `adb devices -l` confirme le Huawei `MAR-LX1A` réellement branché USB (`L2N4C19515006599`, `transport_id:3`). `.\scripts\android\verify-huawei.ps1` rejoué réellement : `gatewayInstalled:true` (APK déjà installée), `identityProofPresent:true` (identité P-256 Android Keystore déjà créée), `deviceId:"device-f86e55f5-7ebf-47e6-a4ac-06e1bb9907d9"`. Activation Wi-Fi volontaire exécutée pour de vrai selon la procédure exacte du runbook (`adb -s L2N4C19515006599 tcpip 5555` puis `adb connect 192.168.1.11:5555`) — un seul appareil présent sur le réseau, rien à rejeter.
- [x] Run:
