...
## 2026-08-08 05:05 | feat/ocr | fallback Tesseract local et sonde vision Mina réussie

- files: `src/providers/tesseract-ocr.mjs`, `src/chat/media-perception.mjs`, `src/ui/main.mjs`, les trois tests OCR associés et le tableau des tâches restantes.
- TDD OCR: le test a d’abord échoué car `src/providers/tesseract-ocr.mjs` était absent et la perception renvoyait `caption:null` après l’échec vision. Après implémentation, les `3` fichiers ciblés passent (`13` tests) et la suite unitaire passe `421` fichiers / `3 395` tests avec `npx vitest run --exclude tests/integration/** --maxWorkers=1 --no-file-parallelism` (`321,17 s`).
- proof OCR live: une image SVG synthétique convertie en PNG a été envoyée au binaire Tesseract local via stdin. La sortie était `{"modelId":"tesseract:eng","text":"MINA OCR TEST","blocks":1}`. Le fournisseur n’utilise ni réseau, ni téléchargement, ni fichier temporaire ; il essaye la vision en premier et ne bascule vers Tesseract qu’en cas d’échec ou de réponse vide.
- languages: le binaire local liste `eng` et `osd`; `fra` n’est pas installé. Le fallback sélectionne donc `eng` sur cette machine, sans installation implicite de données linguistiques.
- proof vision live: Gemma `google/gemma-4-e2b` chargé à `4096` tokens et un flux a répondu via l’adaptateur OpenAI-compatible réellement consommé par Mina pour une image locale de contrôle. La réponse finale a été récupérée dans `message.content`; le raisonnement séparé n’est ni affiché ni consommé. Le flag ne change que dans `.env` local ignoré; `.env.example` reste fermé par défaut.
- Firebase: Nasro a désigné l’identité navigateur `/u/5` comme autorité pour les prochaines opérations Mina. Cette décision n’a déclenché aucune commande Firebase, aucune mutation cloud et ne modifie pas le projet `mina-vision`.
- manual/live: aucune webcam, caméra téléphone, image utilisateur, microphone, donnée Firebase, donnée mail ou donnée externe n’a été utilisée. La sonde OCR et la sonde vision ont employé exclusivement des images de contrôle locales.
- remaining: tester les entrées caméra physiques séparément; provisionner un modèle STT local; réconcilier puis terminer les tâches Android option A, grounding, documents et impression. Le commit code est `75824ae` (`feat(ocr): add local Tesseract fallback`).

## 2026-08-08 08:11 | fix/android | réveil FCM opaque durci et APK debug installés

- files: `android/app/src/main/AndroidManifest.xml`, `MinaChatMessagingService.kt`, `ChatSyncScheduler.kt`, `FcmSyncSignal.kt`, `FirebaseFcmSession.kt`, `ChatEngine.kt` et leurs tests de contrat.
- TDD: le contrat Node a d’abord échoué car `ChatSyncScheduler.kt` était absent. Après l’implémentation, `npx vitest run tests/android-chat-background-contract.test.mjs --maxWorkers=1 --no-file-parallelism` a passé (`1/1`), puis `:app:testDebugUnitTest --tests fr.mina.gateway.chat.FcmSyncSignalTest` a passé (`3/3`). Le lint et l’assemblage `:app:lintDebug :app:assembleDebug` ont aussi passé.
- proof Android: l’APK debug produit a été installé sur le Huawei USB `MAR_LX1A` et le Samsung A71 ADB Wi-Fi `SM-A715F`. Les hash SHA-256 des deux packages `fr.mina.gateway.debug` correspondent au fichier APK local. L’ancienne application sans suffixe n’a pas été remplacée par cette installation ; elle a ensuite été constatée absente du Huawei.
- security: le signal FCM est strictement borné, sans contenu ; Messaging/Analytics restent opt-in. Sans claims Firebase `owner_id`/`device_id` exacts, il est refusé. L’ancien relais Auth anonyme ne satisfait pas ce prérequis : aucun token FCM/FID n’est donc enregistré ni utilisé par ce changement.
- Firebase: la dernière consigne Nasro remplace l’autorité navigateur précédente : utiliser désormais `mina.vision.ai@gmail.com` à l’index `/u/0` pour Mina. Aucune lecture ou écriture Firebase cloud n’a été faite dans cette vague.
- provisioning: l’inspection locale sans affichage de valeur a constaté que `.env` ne contient pas de token BotFather/Telegram et que `TELEGRAM_OWNER_CHAT_ID` y est vide. Après suppression de l’ancienne app, les valeurs Android Keystore doivent être provisionnées localement dans l’application ; elles ne doivent pas être copiées dans Git, l’APK ni ce journal.
- manual/live: aucun message Telegram/SMS, aucune permission caméra/micro, aucune synchronisation FCM réelle et aucun appel Firebase cloud n’ont été exécutés.
- remaining: migration Firebase propriétaire/App Check puis coordonnateurs FID/FCM ; provisioning local des appareils ; parcours Android utilisateur, caméra et transport chat ; tâches Android option A restantes.

## 2026-08-08 08:35 | feat/firebase | règles chat owner/device et Emulator RTDB locaux

- files: `firebase/firestore.rules`, `firebase.storage.rules`, `database.rules.json`, `firestore.indexes.json`, `firebase.json`, le runner Emulator et les tests Firebase chat.
- TDD: le premier test a échoué car `firestore.indexes.json` et `database.rules.json` étaient absents. Le test Emulator a ensuite révélé que Storage lisait un document device dans un project ID différent ; le test a été aligné sur `mina-vision`, puis les règles ont passé sur le même projet émulé.
- commands: `npx vitest run tests/firebase-chat-rules.test.mjs tests/firebase-deployment-config.test.mjs tests/firebase-backup.test.mjs tests/firebase-health.test.mjs tests/firebase-transport.test.mjs --maxWorkers=1 --no-file-parallelism` a passé (`5` fichiers / `13` tests, `5` Emulator-only ignorés hors recette). `npm run test:firebase:emulator` a passé : smoke Firestore/Storage historique puis `6/6` règles chat Auth/Firestore/RTDB/Storage, tous sur `127.0.0.1`.
- proof: les règles owner/device refusent owner, device, tokenVersion, capability, cursors et documents runtime incompatibles ; Storage exige appareil actif/capability pour les attachments et RTDB borne présence/streams. Le chemin backup historique reste couvert par le smoke.
- cloud: aucune règle n’a été déployée, aucune ressource Firebase cloud n’a été modifiée et aucune fonction cloud n’existe encore. Le relais `relay/` historique Auth anonyme demeure explicitement transitionnel jusqu’à la migration Functions/adaptateurs.
- dependency: `firebase-tools@15.24.0` et `@firebase/rules-unit-testing@5.0.1` sont ajoutés uniquement aux dépendances de développement. `npm audit --omit=dev` a constaté 20 vulnérabilités runtime préexistantes ou non attribuées (`11` élevées, `9` modérées) ; aucun `audit fix` n’a été lancé.
- remaining: Functions owner/App Check et adaptateurs owner-scopés (tâches 11–12), puis FID/FCM/Huawei, historique et UI native. Le commit code est `335d8e9` (`feat(firebase): add owner scoped chat rules`).
