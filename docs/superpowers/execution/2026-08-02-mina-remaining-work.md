...
- [ ] **Documents / impression** — `main.mjs` compose réellement la quarantaine/intake des documents et l’impression Windows avec confirmation locale. Le contrôleur urgence est enregistré uniquement lorsque le corpus et le mode réels sont composés. Le renderer expose encore seulement l’état d’urgence, sans action de construction ou d’activation ; les services de parsing, preuves, classification, mémoire/RAG, formulaires, conversion et téléchargement ne sont pas composés dans ce runtime. Leurs handlers échouent maintenant explicitement avec `*_not_configured`, jamais par une valeur `undefined`. La réception physique d’impression et un état terminal spooler vérifié restent non exécutés.
