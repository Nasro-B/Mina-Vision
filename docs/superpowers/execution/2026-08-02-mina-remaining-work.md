...
- [x] Autorité Firebase opérationnelle : la dernière consigne Nasro désigne `mina.vision.ai@gmail.com`, via l’index navigateur `/u/0`, pour les prochaines opérations Mina. Aucun paramètre Firebase ni ressource cloud n’a été modifié par cette décision.
...
- [ ] **Chat natif Android, option A** — Nasro a choisi le chat complet des tâches 13–25. La notification Android n’expose plus le texte de réponse (`2185166`), les règles owner/device sont prouvées localement (`335d8e9`) et le réveil FCM est fail-closed (`071d279`). Il reste la migration du relais Firebase anonyme vers une identité propriétaire/appareil avec custom claims, App Check, Functions et enregistrement FID/FCM explicite ; tant qu’elle n’existe pas, le réveil FCM est volontairement ignoré et aucune tâche Android 13–25 ne peut être déclarée complète.
...
- [ ] **Android utilisateur** — le Huawei USB et le Samsung A71 ADB Wi-Fi ont reçu l’APK debug vérifié. Il reste à prouver le parcours application, le provisioning local, les permissions caméra/micro, SMS et Telegram. L’ancienne application `fr.mina.gateway` a été retirée du Huawei ; ses secrets Android Keystore ont disparu avec elle. Le `.env` local ne contient ni token BotFather/Telegram ni ID propriétaire exploitable pour les restaurer automatiquement : le provisioning doit être ressaisi localement dans l’application, sans les exposer dans le dépôt, l’APK ou le chat. Le test instrumentation isolé passé ne prouve pas ces parcours.
