...
- [ ] **Chat natif Android, option A** — Nasro a choisi le chat complet des tâches 13–25. La notification Android n’expose plus le texte de réponse (`2185166`), les règles owner/device sont prouvées localement (`335d8e9`) et le réveil FCM est fail-closed (`071d279`). La migration notes/PTT PCM chiffrée est désormais partielle : gates Android complètes vertes et `75` tests PC verts, mais aucun appareil n’était attaché pour une recette réelle ; la tâche 21 de voix live n’est pas commencée. Il reste la migration du relais Firebase anonyme vers une identité propriétaire/appareil avec custom claims, App Check, Functions et enregistrement FID/FCM explicite ; tant qu’elle n’existe pas, le réveil FCM est volontairement ignoré et aucune tâche Android 13–25 ne peut être déclarée complète.
